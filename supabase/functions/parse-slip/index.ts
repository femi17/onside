...
const MARKET_KEYS = [
  ...
  "goal_range", "home_goal_range", "away_goal_range",
...
GOAL BOUNDS / GOAL RANGE / MULTIGOALS (CRITICAL): a market betting the TOTAL goals fall inside a RANGE — printed as "Goal Bounds", "Goal Range", or "Multigoals" with a pick like "0-3", "2-3", "1-2", "4+" — maps to market_key "goal_range" (NOT custom, NOT over/under). Put the range EXACTLY as printed in value (e.g. "0-3") and in the label ("Goal Bounds 0-3"). If a TEAM name is attached (the fixture's home team's goal bounds -> home_goal_range; the away team's -> away_goal_range), keep value = the range. A "0-3" goal bounds is a distinct market from Under 3.5 — never convert it.
...
- Correct score / HT-FT / handicap / winning margin: market_label = the market name, value = the exact selection (e.g. "2-1", "1/2", "-1.5"). (Goal range / multigoals / goal bounds is NOT custom — use goal_range per the GOAL BOUNDS rule above.)
