...
WHOLE-NUMBER goal lines (CRITICAL): "Over 1", "Over 2", "Under 3" — any goal line WITHOUT .5 — is a DIFFERENT bet from the .5 line: it PUSHES (voids) when the total lands exactly on the number. NEVER round or convert it to a .5 line. Use market_key "custom" with market_label exactly "Over N goals"/"Under N goals" keeping the printed number (e.g. "Over 1 goals"), value "", line = the printed whole number. If the bet is scoped to a half ("1st Half", "2nd Half", "HT"), keep that scope in the label: "1st half - Over 1 goals". Copy the printed line digit-for-digit into raw_market too.
