-- Split agent notifications into two independent opt-in toggles:
--   agent_picks  = "your agent submitted new picks" (per-run summary, run-strategies)
--   agent_games  = live build-up + settled result for the agent's picked games (the triggers below)
-- Both default OFF. A game the user actually TRACKS is a tickets row and keeps its normal categories.
alter table public.notification_prefs add column if not exists agent_games boolean not null default false;
...
begin
  ...
  perform net.http_post(
    ...
    body := jsonb_build_object(
      -- tracked ticket -> build_up; untracked agent pick -> agent_games (opt-in), separate from the
      -- agent_picks "new picks" summary
...
begin
  ...
  perform net.http_post(
    ...
    headers := jsonb_build_object('Content-Type', 'application/json', 'x-internal-secret', v_secret),
    body := jsonb_build_object(
      'user_id', NEW.user_id, 'category', 'agent_games', 'mute', true,
