-- Mirror of remotely applied migration landed_ping_exclude_over05 (2026-09-13).
-- Over 0.5 never posts to Telegram (owner-directed): the web-push landed alert still fires for
-- over_0_5 (in-app is fine); only the Telegram "IT'S IN!" celebration is suppressed. Adds
-- `market_key <> 'over_0_5'` to the telegram branch of notify_delivery_settled / notify_ticket_settled.
-- (Full function bodies re-created; see 20260912010000_live_landed_telegram_ping.sql for the base.)
...
begin
  ...
  select exists (
    ...
    where t.user_id = NEW.user_id and t.fixture_id = NEW.fixture_id and t.market_key = NEW.market_key
      and coalesce(t.side,'') = coalesce(NEW.side,'') and coalesce(t.line,-1) = coalesce(NEW.line,-1)
  ...
  if v_secret is not null then
    perform net.http_post(
      ...
      body := jsonb_build_object('user_id',NEW.user_id,'category',v_cat,'mute',not v_tracked,
  ...
  if NEW.result = 'won' and v_tracked and coalesce(NEW.market_key,'') <> 'over_0_5' then
    ...
    if v_chat is not null and v_tgtoken is not null then
      perform net.http_post(
        ...
        body := jsonb_build_object('chat_id',v_chat,'disable_web_page_preview',true,
          'text', '⚽ IT''S IN! '||coalesce(NEW.market_label,'Your pick')||' just landed ✅'||E'\n'||coalesce(v_home,'')||' v '||coalesce(v_away,'')),
        timeout_milliseconds := 8000);
    end if;
  end if;
  return NEW;
...
begin
  ...
  if exists (
    ...
    where d.user_id = NEW.user_id and d.fixture_id = NEW.fixture_id and d.market_key = NEW.market_key
      and coalesce(d.side,'') = coalesce(NEW.side,'') and coalesce(d.line,-1) = coalesce(NEW.line,-1)
  ...
  if v_secret is not null then
    perform net.http_post(
      url := 'https://mbrtpetpgsggnlcazhqd.supabase.co/functions/v1/send-push',
      headers := jsonb_build_object('Content-Type','application/json','x-internal-secret',v_secret),
      body := jsonb_build_object('user_id',NEW.user_id,'category','results',
        ...
        'url', case when v_acca then '/accumulators' else '/tracker' end, 'tag','settle-t-'||NEW.id::text),
      timeout_milliseconds := 8000);
  end if;
  if NEW.status = 'won' and coalesce(NEW.market_key,'') <> 'over_0_5' then
    select telegram_chat_id into v_chat from public.profiles where id = NEW.user_id;
    select decrypted_secret into v_tgtoken from vault.decrypted_secrets where name = 'telegram_bot_token' limit 1;
    if v_chat is not null and v_tgtoken is not null then
      perform net.http_post(
        ...
        headers := jsonb_build_object('Content-Type','application/json'),
        body := jsonb_build_object('chat_id',v_chat,'disable_web_page_preview',true,
          'text', '⚽ IT''S IN! '||coalesce(NEW.market_label,NEW.custom_market,'Your pick')||' just landed ✅'||case when v_acca then ' (acca leg)' else '' end||E'\n'||coalesce(v_home,'')||' v '||coalesce(v_away,'')),
        timeout_milliseconds := 8000);
    end if;
  end if;
  return NEW;
