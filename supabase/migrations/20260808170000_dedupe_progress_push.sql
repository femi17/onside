-- Fix double build-up alerts. A tracked agent pick exists as BOTH a deliveries row and a
-- tickets row (source 'agent'); the live poller updates current_value on each, so
-- notify_bet_progress pushed the SAME game moment twice (different tags -> two notifications).
-- The ticket is the user-facing tracked copy, so it keeps pushing; the deliveries/agent_picks
-- trigger now stays silent whenever a matching OPEN ticket mirrors the same bet.
...
begin
  ...
  perform net.http_post(
    ...
    body := jsonb_build_object(
      'user_id', NEW.user_id, 'category', 'build_up',
      'title', v_emoji || ' ' || coalesce(v_home, '') || ' v ' || coalesce(v_away, ''),
      'body', v_body, 'url', '/tracker',
