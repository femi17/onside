-- Route notifications about AGENT-picked games (live build-up + settled result) through the opt-in
-- `agent_picks` category instead of the default-on `build_up`/`results`. Agent deliveries are just
-- suggestions until the user tracks them, so their live-event and result pushes now stay silent unless
-- the user turns on "Agent predictions" (Profile · Notifications). A game the user actually TRACKS
-- exists as a `tickets` row and keeps its normal categories. Also tag these with mute:true so the push
-- carries the "Mute agent alerts" button.
...
begin
  ...
  -- the same bet tracked as a ticket already pushes this exact moment — don't double it
  ...
  -- fixed-line defaults (these markets are always full-match)
  ...
  -- period-aware value: replace the full-match current_value with the period's own count + gate by phase
  if v_period <> 'ft' then
    ...
    if v_period = '1h' then
      if v_status is null or v_status not in ('1H','HT') then return NEW; end if;   -- not this phase
      -- during the 1st half the live count IS the 1st-half count
      v_val := case
    ...
    else  -- 2h (and knockouts continuing past 90)
  ...
  perform net.http_post(
    ...
    body := jsonb_build_object(
      -- a tracked ticket keeps its build-up category; an untracked agent pick routes to agent_picks
      -- (opt-in) so it stays silent unless the user turned "Agent predictions" on
      'user_id', NEW.user_id, 'category', case when v_is_ticket then 'build_up' else 'agent_picks' end,
...
begin
  ...
  perform net.http_post(
    ...
    headers := jsonb_build_object('Content-Type', 'application/json', 'x-internal-secret', v_secret),
    body := jsonb_build_object(
      -- an agent delivery's result is an agent notification (opt-in), not a tracked-bet "result"
      'user_id', NEW.user_id, 'category', 'agent_picks', 'mute', true,
