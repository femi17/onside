-- Goal-bounds bets (goal_range / home_goal_range / away_goal_range — "0-3", "1-3", "4+") were
-- omitted from the build-up push allowlist, so a tracked goal-bound got no live "counting down"
-- alerts the way Under bets do. Add a range branch: an UNDER-style countdown toward the bust
-- point (upper bound + 1) for "A-B" bands, and an over-style build-up for open-ended "N+".
-- current_value is now written live for these markets (poll liveValue), so the trigger fires.
-- The over/under (+corner) path is unchanged; v_desc/v_unit/v_emoji just move above the branch.
...
declare
  ...
  v_is_range boolean;
  v_over boolean;
  v_bv text := coalesce(NEW.bet_value, '');
  v_lo int; v_hi int; v_band text;
...
begin
  ...
  v_is_range := v_mk in ('goal_range', 'home_goal_range', 'away_goal_range');
  if not (v_is_corner or v_is_range or v_mk in
  ...
  v_desc := case v_mk
    when 'home_goals_ou' then 'Home ' when 'away_goals_ou' then 'Away '
    when 'home_goal_range' then 'Home ' when 'away_goal_range' then 'Away ' else '' end;
  v_unit := case when v_is_corner then 'corners' else 'goals' end;
  v_emoji := case when v_is_corner then '🚩' else '⚽' end;

  -- GOAL BOUNDS: like Under, the live risk is the count climbing past the upper bound, so count
  -- DOWN toward the bust point (upper + 1). Open-ended "N+" builds UP toward the floor. ft only.
  if v_is_range then
    if v_bv ~ '^\s*\d+\s*\+' then
      v_needed := (substring(v_bv from '(\d+)'))::int;
      v_remaining := v_needed - v_val;
      if v_remaining <= 0 then return NEW; end if;
      v_body := v_desc || v_needed || '+ ' || v_unit || ' — ' || v_val || ' so far, building up · '
        || v_remaining || ' more to clear';
    elsif v_bv ~ '^\s*\d+\s*-\s*\d+' then
      v_lo := (substring(v_bv from '^\s*(\d+)'))::int;
      v_hi := (substring(v_bv from '^\s*\d+\s*-\s*(\d+)'))::int;
      v_left := (v_hi + 1) - v_val;
      if v_left <= 0 then return NEW; end if;   -- already past the band; the settled push speaks
      v_band := v_lo::text || '–' || v_hi::text;
      v_body := v_desc || v_band || ' ' || v_unit || ' — ' || v_val || ' so far, counting down · '
        || v_left || ' more ends it';
    else
      return NEW;   -- exact "N" or unparseable: no build-up signal
    end if;
    ...
    perform net.http_post(
      ...
      body := jsonb_build_object(
        ...
        'tag', 'prog-' || NEW.id::text || '-range-' || v_val::text
      ),
