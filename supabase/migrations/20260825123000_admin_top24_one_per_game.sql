-- The 24 v2 (owner-ruled): ONE pick per game — each fixture contributes only its best-ranked
-- market, so the shortlist spans 24 different games instead of stacking correlated picks on
-- one fixture. Ranking evidence unchanged (cell → family×band → model, big-edge penalty).
...
begin
  ...
  today as (
    select d.id, d.fixture_id, d.model_prob, d.edge, d.market_prob, d.market_key, d.side, d.line,
  ...
  ranked as (
    select t.id, t.fixture_id, t.market_prob, t.model_prob,
      round((
        coalesce(
  ...
  one_per_game as (
    -- each fixture keeps only its best-ranked market (null fixtures each count as their own game)
    select distinct on (coalesce(r.fixture_id::text, r.id::text))
      r.id, r.p_est, r.basis, r.market_prob, r.model_prob
    from ranked r
    order by coalesce(r.fixture_id::text, r.id::text), r.p_est desc, r.market_prob desc nulls last, r.model_prob desc
  )
  select o.id, o.p_est, o.basis
  from one_per_game o
  order by o.p_est desc, o.market_prob desc nulls last, o.model_prob desc
  limit 24;
end;
$function$;
