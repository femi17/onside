-- A cut acca's unplayed legs stay status='pending' but are swept off the tracker
-- (tracker_hidden=true). Tracked-audience queries must not count them: hidden = not tracked =
-- no game alerts (owner rule: tracking a pick is what opts you into its game alerts).
-- Touches ONLY the audience selection in the two notifiers that missed the filter;
-- notify_ticket_settled already had it.
-- (1/2) notify_fixture_event: kickoff/goal/card/full-time alerts.
