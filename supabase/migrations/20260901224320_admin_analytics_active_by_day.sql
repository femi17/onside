-- admin_analytics: + users.active_by_day — distinct users per UTC day (last 7 days incl. today)
-- across USER-INITIATED actions only: tickets, accas, slip uploads, community posts/comments/
-- reactions, agents built, successful payments, founder-question answers. Deliberately excludes
-- strategies.last_run_at (engine runs would mark every agent owner active every day); the 7d
-- headline keeps its original definition.
