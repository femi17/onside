-- Mirror of remotely applied migration platform_health_add_revenue (2026-09-03).
-- Adds real (post-epoch) money to the daily platform-health snapshot: collected revenue since the
-- go-live revenue_epoch, count of paying customers, and today's successful payments. Small-table
-- reads only (payments), no fixtures scan — IO-safe. The platform-health cloud routine reports these.
...
begin
  ...
  select coalesce(value_ts, '-infinity') into v_epoch from public.app_config where key = 'revenue_epoch';
  if v_epoch is null then v_epoch := '-infinity'; end if;
  ...
  return jsonb_build_object(
    ...
    'monetization', jsonb_build_object(
      ...
      'collected_naira', (select coalesce(sum(amount_kobo),0)/100 from public.payments where status='success' and paid_at >= v_epoch),
      'paying_customers', (select count(distinct user_id) from public.payments where status='success' and paid_at >= v_epoch),
      'paid_today', (select count(*) from public.payments where status='success' and paid_at >= greatest(v_epoch, date_trunc('day', now())))
    ),
