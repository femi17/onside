-- Mirror of remotely applied migration revenue_epoch_reset (2026-09-03).
-- Go-live revenue reset (owner-directed). All payment rows to date are test-mode / pre-live money
-- plus two same-day owner test charges — none of it real customer revenue. Rather than delete payment
-- history (immutable record), gate the collected-revenue metrics behind a resettable epoch:
-- collected_naira and revenue_weekly count only payments with paid_at >= the epoch. Set to now() so
-- the counter starts at zero and only genuine post-go-live money accrues. To reset again later:
--   update app_config set value_ts = now() where key = 'revenue_epoch';
create table if not exists public.app_config (
  key text primary key,
  value_ts timestamptz
);
insert into public.app_config (key, value_ts) values ('revenue_epoch', now())
...
begin
  ...
  -- collected-revenue metrics count only money from this epoch onward (go-live reset)
  select value_ts into v_rev_epoch from public.app_config where key = 'revenue_epoch';
  if v_rev_epoch is null then v_rev_epoch := '-infinity'; end if;
  ...
  return jsonb_build_object(
    ...
    'revenue', jsonb_build_object(
      ...
      'collected_naira', (select coalesce(sum(amount_kobo), 0) / 100 from public.payments where status = 'success' and user_id <> all(v_excl) and paid_at >= v_rev_epoch)
    ),
    ...
    'revenue_weekly', (select coalesce(jsonb_agg(jsonb_build_object('w', w, 'amount', amt, 'cum', cum) order by w), '[]'::jsonb)
        from (
          with wk as (select date_trunc('week', paid_at)::date w, sum(amount_kobo) / 100.0 amt from public.payments where status = 'success' and user_id <> all(v_excl) and paid_at >= v_rev_epoch group by 1)
