-- Add the "Lock Acca" line to the School forward-test RPC: the best DC 1X near-locks in bettable
-- leagues (odds <= 1.35), 2-3 legs/day, with a per-leg market swap — a crushing favourite
-- (DC < 1.20 AND home <= 1.35) is bet as HOME WIN for the better price; everything else stays DC 1X.
...
begin
  ...
  union all
  select c.* from dc c where c.rnk <= 3
  union all
