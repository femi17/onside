-- Onside Double: draft-strategy deliveries (quick-acca pool material, paper research bets) were
-- eligible as legs — the pool join carried no status filter, so the flagship banker could pick
-- from picks that are invisible everywhere else (owner-spotted: a "⚡ Quick acca" leg with no
-- visible origin; yesterday's doubles carried two 📄 Paper legs). The pool now requires the
-- source strategy to NOT be a draft. Contaminated doubles for the apply-day were deleted after
-- the patch so the next builder tick recreates them from the clean pool. Mirror of remotely
-- applied migration onside_double_no_drafts.
...
begin
  ...
  loop
    ...
    with pool as (
      ...
      -- drafts (quick-acca pool rows, paper research bets) are NOT agents — never Double legs
      join public.strategies st on st.id = d.strategy_id and st.status <> 'draft'
...
-- repair: apply-day doubles containing a draft-sourced leg are removed; the next builder tick
-- recreates them from the clean pool
delete from public.onside_double od
where od.set_date = (now() at time zone 'Africa/Lagos')::date
  and exists (
    select 1 from jsonb_array_elements(od.legs) leg
    join public.deliveries d on d.id = (leg->>'delivery_id')::uuid
