-- Global model-band calibration for the feed's % colour coding (owner-ruled 2026-08-20).
-- Returns ONLY aggregates (no per-user rows leak through RLS): the same
-- (market family x 10-point model_prob band) cells the engine's bandVeto studies.
-- Family buckets mirror marketGroupOf in run-strategies.
create or replace function public.band_calibration()
returns table(family text, band int, n bigint, won bigint, prob_sum numeric)
language sql
security definer
...
as $$
  select
    ...
    least(9, floor(model_prob * 10))::int as band,
    count(*) as n,
