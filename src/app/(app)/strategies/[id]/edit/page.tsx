...
export default async function EditStrategyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: profile }, { data: leaguesRaw }, { count: existingCount }, { data: strategy }] = await Promise.all([
    ...
    supabase
      .from("strategies")
      .select(
        "id, name, status, market_key, market_label, custom_market, side, line, period, bet_value, rule_text, rule_parsed, kickoff_at, kickoff_until, league_ids, league_mode, selectivity, min_edge, min_odds, max_odds, max_per_prediction, deliver_at, target_day, channels, learning, markets"
      )
      .eq("id", id)
      .eq("user_id", user.id)
      .maybeSingle(),
